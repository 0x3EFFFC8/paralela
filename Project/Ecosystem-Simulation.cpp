...
void processRabbits(int currentGen) {
    ...
    for (Entity* rabbit : rabbitsToProcess) {
        ...
        if (!validMoves.empty()) {
            ...
            if ((currentGen - rabbit->birthGen) >= GEN_PROC_RABBITS) {
                Entity* newRabbit = new Entity{RABBIT, rabbit->x, rabbit->y, currentGen + 1, -1};
                destinationMap[{rabbit->x, rabbit->y}].push_back(newRabbit);
                rabbit->birthGen = currentGen + 1;
            }
            rabbit->x = newPos.first;
            rabbit->y = newPos.second;
        }    
        destinationMap[{rabbit->x, rabbit->y}].push_back(rabbit);
    }

    grid.clear();
    
    for (auto& [pos, rabbitList] : destinationMap) {
        auto oldest = max_element(rabbitList.begin(), rabbitList.end(), [currentGen](Entity* a, Entity* b) {
...
void processFoxes(int currentGen) {
    ...
    for (Entity* fox : foxesToProcess) {
        ...
        if (!rabbitMoves.empty()) {
            ...
            if (rabbit) rabbits.erase(remove(rabbits.begin(), rabbits.end(), rabbit), rabbits.end());

            if ((currentGen - fox->birthGen) >= GEN_PROC_FOXES) {
                Entity* newFox = new Entity{FOX, fox->x, fox->y, currentGen + 1, currentGen + 1};
                destinationMap[{fox->x, fox->y}].push_back(newFox);
                fox->birthGen = currentGen + 1;
            }
            fox->x = newPos.first;
            fox->y = newPos.second;
            fox->lastEat = currentGen + 1;
            destinationMap[{fox->x,fox->y}].push_back(fox);
            ateRabbit = true;
        }
