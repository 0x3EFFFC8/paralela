...
void * threadAddMatrices(void *arg){
  ThreadData* data = (ThreadData*)arg;
  addMatrices(data->matrixA,data->matrixB,data->matrixC,SIZE);
  pthread_exit(NULL);
}

void * threadAsubMatrices(void *arg){
  ThreadData *data = (ThreadData*)arg;
  subMatrices(data->matrixA,data->matrixB,data->matrixC,SIZE);
  pthread_exit(NULL);
}
...
int main(){
  ...
  pthread_t pthreadSum,pthreadDiff;
  srand(time(NULL));
  GMatrix(matrixA);
  GMatrix(matrixB);

  ThreadData sumAB = {matrixA,matrixB,matrixC};
  ThreadData subAB = {matrixA,matrixB,matrixD};

  pthread_create(&pthreadSum,NULL,threadAddMatrices,(void*)&sumAB);
  pthread_create(&pthreadDiff,NULL,threadAsubMatrices,(void*)&subAB);
  
  pthread_join(pthreadSum,NULL);
  pthread_join(pthreadDiff,NULL);
